...
public class MinIO {
    ...
    public boolean uploadFileEnabled = Boolean.parseBoolean(getProperty("MinIO.enableUploadFile"));
    ...
    public void deleteAll() throws IOException, NoSuchAlgorithmException, InvalidKeyException {
        try {
            if (minioClient == null)
                initializeMinIOClient();

            Iterable<Result<Item>> results = minioClient.listObjects("sys","selenium-core/stag/");
            for (Result<Item> result : results) {
                if (result.get().objectName().startsWith("selenium-core/stag/")) {
                    minioClient.removeObject("sys",result.get().objectName());
                    logger.info(result.get().objectName() + " is removed successfully");
                }
            }
